...
- **Detection covers values, JSON numbers, and object keys.** Base64/encoded values and URL query strings are documented exclusions (see threat model). On the **response direction only**, Haechi's own transform markers (`[TOKEN:…]`, `[HAECHI_ENC:…]`, `[REDACTED:…]`) are skipped so a tokenized round-trip echoed by the model isn't re-flagged as a secret — this is response-only on purpose (a request containing a marker-shaped string is scanned normally, so it can't be used to smuggle a secret). The KR phone rule rejects a bare, separator-less digit run that doesn't start with the trunk `0` (so a unix timestamp / id isn't a "phone").
