...
export const SCORED_TYPES = ["email", "phone", "kr_rrn", "card", "api_key", "secret", "injection", "us_ssn", "iban", "jp_mynumber", "fr_nir", "es_dni", "uk_nino", "it_codice_fiscale", "sg_nric", "in_aadhaar", "de_steuer_id", "nl_bsn"];
...
export function buildBaseline(scores) {
  ...
  return {
    note: "Pinned per-type TP/FP/FN baseline for the detection regression gate (WS2a corpus, WS2b rules, WS2c context anchors, WS2d Unicode-evasion normalization, the international-PII expansion: jp_mynumber/fr_nir/es_dni/uk_nino then the EU/Asia national-ID expansion it_codice_fiscale/sg_nric/in_aadhaar/de_steuer_id/nl_bsn, plus the cloud/SaaS credential expansion: OpenAI sk- / Anthropic sk-ant- / Google OAuth GOCSPX- / npm npm_ (secret), SendGrid SG. / Twilio AC|SK+32hex (api_key), Stripe sk_live_ already covered as api_key by the underscore openai-like-key rule, and the Azure Storage AccountKey= / Twilio auth_token assignment-vocab additions — each anchored on prefix+charclass+length, with a push-safe low-entropy synthetic positive + a near-miss hard-negative + a random-base62 no-fire guard; recall held at 1.0 and secret precision rose as TP grew with no new FP). Records the CURRENT state — WS2b closed the credential/PII coverage gaps (AWS/GitHub/Google/Slack keys, JWT, PEM, US SSN, IBAN, E.164/US phone are true-positives), so recall is at 1 across types. WS2c added a word-boundary anchor to the kr-phone rule that stops a phone matching as a SUBSTRING of a longer hex/dashed run (UUID tail) — a new hard-negative the corpus proves; recall held (no FN reintroduced). WS2d added NFKC normalization in detectEntry so full-width / mathematical / confusable evasion no longer defeats every rule: the new full-width-digit card, full-width KR phone, full-width-@ email, full-width-body sk_ key (same-length NFKC, exact-span redaction) and the mathematical-bold KR RRN (length-divergent NFKC, fail-closed whole-leaf detection) are now true-positives, so TP rose per type while recall held at 1 and the new ligature/ascii hard-negatives added NO false positives (precision for card/phone improved as TP grew with no new FP). The international-PII expansion added four validator-anchored types with a synthetic positive + a checksum/format near-miss each: jp_mynumber (mod-11 check digit), fr_nir (97-control key with Corsica 2A/2B substitution), es_dni (mod-23 check letter, NIE X/Y/Z→0/1/2), and uk_nino (format-only invalid-prefix exclusions — no checksum exists). fr_nir (mod-97 over a 15-digit structured run) and es_dni (mod-23 plus a required check letter) join HARD_BLOCK_TYPES — strong anchors and rare shapes, so a match is effectively a true positive. jp_mynumber and uk_nino stay DIAL-ELIGIBLE: a lone mod-11 check digit passes ~1/11 of common 12-digit runs and uk_nino has no checksum at all, so an operator can allowlist a benign FP (both still detect + block by default). The EU/Asia national-ID expansion added five more validator-anchored types, each with a SYNTHETIC/public-vector positive + a checksum/check-char near-miss: it_codice_fiscale (16-char mixed alpha+digit, mod-26 check character), sg_nric (letter prefix [STFGM] + 7 digits + weighted-sum check letter), in_aadhaar (12 digits + Verhoeff), de_steuer_id (11 digits + ISO 7064 MOD 11,10 + an exactly-one-repeated-digit structural test), and nl_bsn (9 digits + the 11-proef weighted mod-11). The HARD-BLOCK decision was DATA-DRIVEN — collision rate measured by brute-forcing ~200k random same-shaped values per validator: it_codice_fiscale (~3.8%, but a rare mixed alpha+digit shape with a NON-numeric structural anchor) and sg_nric (~3.9%, with TWO non-numeric anchors — prefix letter + check letter) join HARD_BLOCK_TYPES because a benign FP is implausible by construction. in_aadhaar (~9.9%, a bare 12-digit run, the jp_mynumber footgun), de_steuer_id (~0.37% but a bare 11-digit run with NO non-numeric anchor over a common length), and nl_bsn (~9.1%, a bare 9-digit run, the clearest footgun) stay DIAL-ELIGIBLE so an operator can clear a benign id-FP (all three still detect + block by default). Each new type lands at precision 1.0 / recall 1.0 and adds no false positive to any existing type. The remaining false positives are LEFT by design and stay baked in: phone order-id and a Luhn-passing card order-number are structurally identical to a real bare KR mobile / real card so an anchor would cost recall (card is hard-block); Bearer-in-prose is left because detection runs per string leaf and a real {\"Authorization\":\"Bearer <token>\"} payload walks to a bare \"Bearer <token>\" leaf, so an Authorization-context anchor would MISS the realistic case (a recall regression on the hard-block `secret` type); the password-placeholder is arguably a true positive. An operator clears a benign SOFT-type FP (e.g. the phone order-id, a uk_nino format-only FP, or an in_aadhaar/de_steuer_id/nl_bsn bare-digit-id FP) via filters.minConfidence / filters.allowlist; those controls can NEVER suppress a hard-block type (secret/api_key/kr_rrn/card/fr_nir/es_dni/it_codice_fiscale/sg_nric — jp_mynumber/uk_nino/in_aadhaar/de_steuer_id/nl_bsn are dial-eligible). The gate fails only on a regression below these numbers. Regenerate with: node scripts/bench-detection.mjs --write-baseline.",
