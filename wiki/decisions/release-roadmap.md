...
| 0.8.0 (implemented; publish pending) | Ecosystem foundation + satellites | Implemented 2026-06-10 (PRs #27–#29): npm workspaces monorepo (root self-member `["."]` + satellite peer/dev dual core dep), `haechi-crypto-kms` (real AWS KMS, AWS SDK optional peer) + `haechi-auth-jwt` (headless JWKS bearer), each with a provenance-attested publish workflow; core zero-dep via packed-manifest + satellite-packaging gates. **Publish gated on the operator creating the `@haechi` org + Trusted Publishers** ([[packaging-and-distribution]]); core can publish on its existing TP. Design: `release-0.8-implementation-scope.md` |
