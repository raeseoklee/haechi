...
| 0.8.0 | Ecosystem foundation + satellites | npm org `@haechi/*`, npm workspaces monorepo (verified mechanic: root self-member `["."]` + satellite peer/dev dual core dep), publish `@haechi/crypto-kms` (real AWS KMS) and `@haechi/auth-jwt` (headless JWKS bearer). Core stays zero-dep via a packed-manifest CI gate. Design: `release-0.8-implementation-scope.md` ([[packaging-and-distribution]]) |
