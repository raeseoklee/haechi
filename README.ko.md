...
Haechi는 OpenAI 호환 서버, vLLM, Ollama, llama.cpp, 그리고 Anthropic Messages API용 프로토콜 adapter 프리셋을 제공합니다.
...
| `target.type` / `target.adapter` | `llm-http` / `openai-compatible` | upstream 프로토콜: `openai-compatible`, `vllm-openai`, `ollama`, `llama-cpp`, `anthropic`. 알 수 없는 type은 fail-closed로 처리됩니다 |
