...
**proxy에는 아직 클라이언트 인증이 없습니다**(0.6 계획). 포트에 접근할 수 있는 사람은 누구나 upstream과 token round-trip 경로를 쓸 수 있습니다. `--allow-remote-bind`는 명시적인 네트워크 통제가 있을 때만 사용하세요.
...
- 0.1 crypto provider는 Node `crypto`의 AES-256-GCM과 로컬 소프트웨어 키를 사용합니다.
