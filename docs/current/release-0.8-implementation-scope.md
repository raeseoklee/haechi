...
### 2.3 `@haechi/crypto-kms` (publish + real KMS clients)

- Promote the 0.7 reference (`createKmsCryptoProvider` envelope encryption + `createInMemoryKms`) into the published package, switching its inlined `canonicalize` to `import { canonicalize } from "haechi/crypto"` (§2.1).
- Add a **real KMS client** as the satellite's own dependency: `createAwsKmsClient({ keyId, region })` over `@aws-sdk/client-kms` (`GenerateDataKey` / `Decrypt`, plus HKDF for `deriveHmacKey`). The AWS SDK is a dependency **of the satellite only** and never enters core's SBOM. The in-memory client remains for tests/examples and needs no SDK. (A Vault client is explicitly 0.9+, to keep 0.8 to one real backend.)
- The satellite's CI runs `assertCryptoProviderConformance` (imported from `haechi/crypto` via the workspace symlink) against the in-memory client and a **mocked** AWS client (no live AWS calls in CI). The mock must be a faithful envelope: `GenerateDataKey` returns `{ Plaintext, CiphertextBlob }`; `Decrypt` returns the original plaintext only for a blob this key wrapped, and **rejects** a blob wrapped by a different key/region (cross-tenant isolation) and corrupted/truncated blobs. A trivial always-succeeds stub is insufficient and the conformance suite must exercise these rejection paths. Live `createAwsKmsClient` validation against a sandbox KMS key is an **out-of-CI integration test** (documented, not gating).
...
- In-memory **and mocked-AWS** clients pass `assertCryptoProviderConformance`, including the cross-key/corrupted-blob **rejection** paths; end-to-end through `createRuntime` (encrypt + tokenization round-trip).
- The published manifest sets `publishConfig.access: public`; the satellite tarball carries `@aws-sdk/client-kms` as its **own** dependency while core's tarball stays zero-dep.
