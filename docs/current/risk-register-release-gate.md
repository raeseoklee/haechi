...
The authoritative itemized register is `docs/current/code-review-risk-register-2026-06-16.md`. This section is the release-gate summary. `P0-CR-001`, `P1-CR-002`, `P1-CR-003`, `P1-CR-004`, and `P1-CR-005` are **Resolved** (the proxy security patch, the SSRF IPv4-mapped normalization, and the streaming-inspection text fix, toward 1.3.1); `P2-CR-013` is **Resolved** with the same streaming change. The remaining open P2 items still block any new GitHub release tag or npm publish until they are resolved or explicitly accepted with owner rationale.
