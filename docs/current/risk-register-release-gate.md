...
| 0.8.0 | Ecosystem foundation + satellites | npm org (`@haechi/*`), npm workspaces monorepo (root self-member `["."]` + `satellites/*`), publish `@haechi/crypto-kms` (real AWS KMS client) and `@haechi/auth-jwt` (headless JWKS bearer verification). Core stays zero runtime dependency (CI no-leak + zero-dep gate). See `docs/current/release-0.8-implementation-scope.md` |
