...
**2026-06-16 current gate overlay:** a full code review opened the risk register at `docs/current/code-review-risk-register-2026-06-16.md`. The review found one P0 credential-boundary leak, four P1 release-blocking issues, and eight P2 hardening/test gaps. **All 13 `P*-CR-*` findings are now Resolved on `main` (§5.7).** The remediation is merged but not yet published — the released `haechi@1.3.0` artifact still carries the pre-fix behavior — so new release tags and npm publishes stay blocked under the G9 gate until the 1.3.1 cut ships the fixes (the version bump, the G9 flip to Pass, and the attested publish).
...
| GitHub release/tag | Ready at the 1.3.1 cut | All §5.7 findings are Resolved on `main`; the next tag is the `v1.3.1` remediation cut that flips G9 to Pass |
| npm stable | Blocked until 1.3.1 publishes | The stable-line baseline is met and the code-review remediation is merged; the next publish is the `haechi@1.3.1` attested cut shipping the fixes |
| Production use | Operator-gated; upgrade to 1.3.1 when published | Supported only with operator network controls, authz/authn, and key custody; the proxy header-boundary fix (P0-CR-001) is merged but ships in 1.3.1 — operators on `haechi@1.3.0` should not route sensitive third-party upstream traffic through the proxy until they upgrade to 1.3.1 |
...
| G9 | 2026-06-16 full code-review remediation gate | `P0-CR-001` and `P1-CR-002` through `P1-CR-005` resolved or formally accepted; P2 items either resolved or scheduled with explicit non-blocking rationale; linked register updated. **All 13 `P*-CR-*` findings are now Resolved on `main` (§5.7); remediation is complete.** Gate clears at the 1.3.1 version cut (version bump + this flip + attested publish). | Blocked (clears at 1.3.1 cut) |
...
The authoritative itemized register is `docs/current/code-review-risk-register-2026-06-16.md`. This section is the release-gate summary. **All 13 findings are now Resolved** (toward 1.3.1): the P0 + four P1s (proxy header-boundary patch, SSRF IPv4-mapped normalization, response-header/streaming bounds, streaming-inspection text fix) and all eight P2s (CR-006 mcp-wrap stderr filter, CR-007 init key-file validation, CR-008 satellite `manifest.bin` check, CR-009 auth-throw regression test, CR-010 process-sandbox quota tests, CR-011 audit middle-tamper tests, CR-012 vault IPv6 tests, CR-013 SSE multi-line `data:`). No remediation item now blocks release; the **G9** gate clears at the 1.3.1 version cut (the version bump + formal gate flip + attested publish).
