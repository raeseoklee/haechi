...
| Response-metadata false positives | Response inspection in `enforce` blocks legitimate completions because envelope metadata looks PII/secret-shaped (e.g. a unix-timestamp `created` matching the phone rule) | The KR phone rule now ignores bare separator-less non-`0`-led digit runs; ids like `chatcmpl-…` are not secret-shaped. **Residual:** a long Luhn-passing numeric field can still match `card`. For real upstreams prefer `responseProtection.mode: report-only` (detect + audit, no block) or tune the response policy |
