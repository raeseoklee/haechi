...
| 0.8.0 | ecosystem foundation + satellites | npm org(`@haechi/*`), npm workspaces 모노레포(루트 자기참조 `["."]` + `satellites/*`), `@haechi/crypto-kms`(실제 AWS KMS 클라이언트)와 `@haechi/auth-jwt`(헤드리스 JWKS bearer 검증) 배포. core는 zero runtime dependency 유지(CI no-leak + zero-dep 게이트). `docs/current/release-0.8-implementation-scope.md` 참조 |
