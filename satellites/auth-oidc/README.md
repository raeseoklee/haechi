...
The session store is pluggable via `sessionStore` (default: the in-memory `createInMemorySessionStore()`), but it **MUST be synchronous in all of `get`/`set`/`delete`** — each completes in the same event-loop turn and must **never** return a `Promise`. The broker reads them and acts synchronously in `authenticate()`, `logout()`, `evictSession()`, and the refresh resurrection guard, whose get-then-set atomicity (a logged-out / superseded session must never be silently resurrected by a concurrent refresh) depends on a synchronous read. Two fail-open vectors close this off:

- An async `get()` returning a `Promise` would make a truthy `Promise` look like a valid session (an arbitrary cookie authenticates).
- An async/deferred `delete()` would let `/auth/logout` return `200` while the session is still live in the store (a replayed cookie keeps authenticating).

So **all three methods are probed at construction** (`normalizeOidcConfig` probes a reserved id and throws fail-closed if any returns a thenable or throws), `authenticate()` carries a defense-in-depth thenable guard, and **logout/eviction confirm the session is provably gone in the same turn** (`destroySessionSync`): if destruction can't be confirmed, logout **fails closed with `500`, never a `200` "logged_out"**, and the breach is audited as `store_async`. To use a shared backend, wrap it in a synchronous in-process cache.
...
- **`sessionStore`** (기본 `createInMemorySessionStore()`): 교체 가능하지만 **`get`/`set`/`delete` 모두 반드시 동기(synchronous)** 여야 합니다 — 각 호출이 같은 이벤트 루프 턴에서 끝나야 하며 **절대 `Promise`를 반환하면 안 됩니다**. 브로커는 `authenticate()`·`logout()`·`evictSession()`·refresh resurrection 가드에서 이들을 동기로 읽고 처리하며, get-then-set 원자성(로그아웃·교체된 세션이 동시 refresh로 조용히 되살아나지 않도록)이 동기 동작에 의존합니다. fail-open 경로는 둘입니다: (1) async `get()`이 `Promise`를 반환하면 truthy `Promise`가 유효 세션처럼 보여 임의 쿠키가 인증되고, (2) async/지연 `delete()`면 `/auth/logout`이 `200`을 반환한 직후에도 세션이 스토어에 살아 있어 쿠키 replay가 계속 인증됩니다. 그래서 **세 메서드 모두 구성 시 probe**(예약 id로 호출해 thenable 반환·throw면 fail-closed)하고, `authenticate()`에 thenable 방어선을 두며, **logout/eviction은 같은 턴에 세션이 확실히 제거됐는지 확인**(`destroySessionSync`)합니다 — 확인 불가면 logout은 **`200`이 아니라 `500`으로 fail-closed**하고 위반은 `store_async`로 감사 기록됩니다. 공유 백엔드는 동기 in-process 캐시로 감싸 쓰세요. 네이티브 async 공유 세션 스토어는 resurrection 가드용 CAS/버전/tombstone 계약이 필요하므로 **현재 범위 밖**(향후 과제)입니다.
